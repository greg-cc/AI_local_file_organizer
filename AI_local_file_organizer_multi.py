...
def initialize_models():
    ...
    # --- Multicore configuration for PyTorch ---
    num_cores = os.cpu_count()
    if device == "cpu":
        # By default, PyTorch might not use all cores. Explicitly set threads for better performance.
        torch.set_num_threads(num_cores)
        print(f"{Fore.CYAN}CPU Cores detected: {num_cores}. PyTorch will be using {torch.get_num_threads()} threads for CPU-based tasks.
...
def summarize_text(text, max_chunk_length, max_summary_length, min_summary_length, timeout_seconds, show_verbiage):
    ...
    if show_verbiage:
        ...
        print(f"\n{Fore.MAGENTA}--- Chunk 1 Verbiage ({len(words)} words) ---{Style.RESET_ALL}")
...
def is_file_cached_for_processing(file_path, existing_files_cache, file_management_settings, settings, categories):
    """
    Checks if a file has already been processed based on the cache of existing files.
    This replaces the redundant logic in the main loop and process_single_file.
    """
    file_name_no_ext = os.path.splitext(os.path.basename(file_path))[0]
...
{Style.RESET_ALL}")
    ...
    for special_cat in ["contains no text", "unreadable file"]:
        ...
        if expected_output_name in existing_files_cache:
            print(f"{Fore.LIGHTBLACK_EX}Skipping pre-filtered: '{os.path.basename(file_path)}' found in '{sanitized_cat}' subfolder.{Style.RESET_ALL}")
            return True
...
file_management_settings, settings_to_save)
        ...
        if settings_to_save['file_action_choice'] != 'none':
            ...
            for file_path in files_to_process:
                if not is_file_cached_for_processing(file_path, existing_files_cache, file_management_settings, settings_to_save, CATEGORIES):
